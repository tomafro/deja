...
@test "run --watch-path" {
  ...
  deja run --watch-path $folder -- example-command
  assert_success_with_uuid "runs command and returns result"
  ...
  deja run --watch-path $other_folder -- example-command
...
@test "run --watch-scope" {
  deja run --watch-scope a -- example-command
  assert_success_with_uuid "runs command and returns result"
...
@test "run --watch-env" {
  ENV_A=1 deja run --watch-env ENV_A -- example-command
  assert_success_with_uuid "runs command and returns result"
...
@test "run --cache-for" {
  deja run --cache-for 1s -- example-command
  assert_success_with_uuid "runs command and returns result"
...
@test "run --exclude-pwd" {
  ...
  deja run --debug --exclude-pwd -- example-command
...
@test "run (error: unable to write to cache)" {
  deja run --cache /missing/folder -- example-command
  assert_handled_failure "fails when unknown command"
...
@test "run --look-back (error: invalid duration)" {
  deja run --look-back 1xyz -- example-command
...
@test "read --cache-miss-exit-code" {
  deja read --cache-miss-exit-code 123 -- example-command
  assert_handled_failure "fails when no result cached"
...
@test "explain" {
  deja explain -- example-command
  assert_success
}

@test "hash" {
  deja hash -- example-command
  assert_success
  ...
  deja hash --watch-path src -- example-command
